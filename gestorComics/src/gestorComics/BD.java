...
public class BD implements IBD {
	...
	public void insertarVineta(Vineta vineta, int comic) throws ExcepcionBD {
		// Convertimos la imagen a un archivo png en memoria ( stream de array de bytes
		// y a un inputstream ) para subir
		...
		try {
			...
			if (enlaces == 0) { // viñeta nueva, insertar imagen y asignar ID
				...
				if (vineta.getID() == -1) {
					...
					psmnt = con.prepareStatement("INSERT INTO VINETA (NOMBRE, ID, IMAGEN, ENLACES) VALUES (?,?,?)");
				...
				psmnt = con.prepareStatement("UPDATE VINETA SET ENLACES=? WHERE ID=?");
			...
			psmnt = con.prepareStatement("INSERT INTO COMIC_VINETA (VINETA_ID, COMIC_ID) VALUES(?,?)");
	...
	public int getEnlaces(int vineta) {
		...
		if (vineta == -1)
			...
			try {
				psmnt = con.prepareStatement("SELECT ENLACES FROM VINETA WHERE ID = ?");
				if (psmnt.getFetchSize() == 0)
					enlaces = 0;
				else {
	...
	public void insertarComic(Comic comic) throws ExcepcionBD {
		...
		try {
			...
			if (comic.getID() == -1)
				comic.setID(++ultimoIdComic);

			psmnt = con.prepareStatement("INSERT INTO COMIC (ID, NOMBRE, PORTADA) VALUES (?,?,?)");
	...
	private Vineta getVineta(int id) throws ExcepcionBD {
		...
		PreparedStatement psmnt = null;
		try {
			psmnt = con.prepareStatement("SELECT IMAGEN, NOMBRE, ID" + " FROM VINETA WHERE ID=" + id);
			ResultSet rs = psmnt.executeQuery();
	...
	/*
	 * @Override public Vineta getPortada(Comic comic) { SELECT * FROM COMIC,
	 * COMIC_VINETA WHERE COMIC.PORTADA = ? }
	 */
	...
	public List<Vineta> getVinetas(int comic) throws ExcepcionBD {
		...
		try {
			...
			psmnt = con.prepareStatement("SELECT V.IMAGEN, V.NOMBRE, V.ID FROM VINETA V, COMIC_VINETA CV"
	...
	public int getUltimoIdComic() {
		...
		PreparedStatement psmnt = null;
		try {
			psmnt = con.prepareStatement("SELECT MAX (ID) FROM COMIC");
			ResultSet rs = psmnt.executeQuery();
			if (rs.next())
	...
	public int getUltimoIdVineta() {
		...
		PreparedStatement psmnt = null;
		try {
			psmnt = con.prepareStatement("SELECT MAX (ID) FROM VINETA");
			ResultSet rs = psmnt.executeQuery();
			if (rs.next())
				resultado = rs.getInt(1);
	...
	public void setPortadaComic(Vineta v, Comic c) {
		PreparedStatement psmnt = null;
		try {
			psmnt = con.prepareStatement("UPDATE COMIC SET PORTADA=? WHERE ID=?");
			if (v != null)
				psmnt.setInt(1, v.getID());
			else
				psmnt.setNull(1, java.sql.Types.NULL);
			...
			throw new ExcepcionBD(
					"Error al poner portada (ID: " + v + ") en comic (ID: " + c + ") (" + e.getMessage() + ")");
		} finally {
			try {
				psmnt.close();
			} catch (SQLException e) {
	...
	public void renombrarVineta(Vineta v, String n) throws ExcepcionBD {
		PreparedStatement psmnt = null;
		try {
			psmnt = con.prepareStatement("UPDATE VINETA SET NOMBRE=? WHERE ID=?");
			psmnt.setString(1, n);
	...
	public void renombrarComic(int c, String n) throws ExcepcionBD {
		PreparedStatement psmnt = null;
		try {
			psmnt = con.prepareStatement("UPDATE COMIC SET NOMBRE = ? WHERE ID = ?");
			psmnt.setString(1, n);
			psmnt.setInt(2, c);
			psmnt.executeUpdate();
		} catch (SQLException e) {
	...
	/*
	 * Decrementar en uno el contador de enlaces y cuando sea 1, Borrar la tupla de
	 * la base de datos
	 * 
	 */
	@Override
	public void borrarVineta(int v, int c) throws ExcepcionBD {
		...
		try {
			...
			if (enlaces == 1) {
				psmnt = con.prepareStatement("DELETE FROM VINETA WHERE ID = ?");
				psmnt.setInt(1, v);
				psmnt.execute();
			} else if (enlaces >= 1) {
				psmnt = con.prepareStatement("UPDATE VINETA SET ENLACES=? WHERE ID = ?");
				...
				throw new ExcepcionBD("Error de número de enlaces de viñeta con ID: " + v);
	...
	public void borrarComic(int c) throws ExcepcionBD {
		...
		PreparedStatement psmnt = null;
		try {
			psmnt = con.prepareStatement("DELETE FROM COMIC WHERE ID = ?");
			psmnt.setInt(1, c);
			psmnt.execute();
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public Vineta getPortada(int c) throws ExcepcionBD {
		...
		// } catch (SQLException e) {
		// e.printStackTrace();
		// throw new ExcepcionBD("Error al borrar comic (ID: "+c+")
		// ("+e.getMessage()+")");
		// }finally {
		// try {
		// psmnt.close();
	...
	public void insertarVinetas(List<Vineta> vs, int c) {
		PreparedStatement psmnt = null;
		try {
			psmnt = con.prepareStatement("INSERT INTO VINETA (ID,NOMBRE,ENLACES,IMAGEN) VALUES (?,?,?,?)");
	...
	public void actualizarImagenVineta(Vineta v) {
		...
		PreparedStatement psmnt = null;
		try {
			psmnt = con.prepareStatement("UPDATE VINETA SET IMAGEN = ? WHERE ID = ?");
	...
	public void insertarBoceto(Vineta boceto, Comic comic) {
		PreparedStatement psmnt = null;
		try {
			psmnt = con.prepareStatement("INSERT INTO BOCETO (ID,COMIC_ID,IMAGEN) VALUES (?,?,?)");
			...
			try {
				...
				throw new ExcepcionBD(
						"Error al insertar boceto (ID: " + boceto.getID() + ") " + "(" + e.getMessage() + ")");
			}
	...
	public List<Vineta> getBocetos(int iD) {
		...
		PreparedStatement psmnt = null;
		try {
			psmnt = con.prepareStatement("SELECT * FROM BOCETO WHERE ID = ?");
			psmnt.setInt(1, iD);
			ResultSet rs = psmnt.executeQuery();
			if (rs.next()) {
				try {
	...
	public void insertarAnotacion(Comic comic, Vineta vineta, Vineta boceto, String comentario) {
		...
		try {
			...
			if (vineta != null)
				...
				psmnt.setNull(2, java.sql.Types.NULL);

			if (boceto != null)
				psmnt.setInt(3, boceto.ID);
			else
				psmnt.setNull(3, java.sql.Types.NULL);
